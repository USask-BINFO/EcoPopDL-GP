...
class GxE_Transformer_Tensor(nn.Module):
    ...
    def __init__(
        ...
        homeolog_group_token_layers: int = 1,
        ...
        use_homeolog_auxiliary: bool = False,
        homeolog_aux_hidden_dim: Optional[int] = None,
        homeolog_aux_stop_grad_target: bool = True,
        homeolog_aux_cosine_weight: float = 1.0,
        homeolog_aux_huber_weight: float = 0.25,
    ):
        super().__init__()
        self.embed_dim = int(embed_dim)
        ...
        # Homeolog / homology channels (optional)
        self.hom_has_channel_idx = hom_has_channel_idx
        self.hom_gid_channel_idx = hom_gid_channel_idx
        self.hom_hash_indices = hom_hash_indices
        ...
        self.sg_channel_idx = sg_channel_idx
        self.use_homeolog_context = bool(use_homeolog_context)
        self.homeolog_context_mode = str(homeolog_context_mode)
        self.homeolog_context_scale = float(homeolog_context_scale)
        self.homeolog_context_add_code_embed = bool(homeolog_context_add_code_embed)
        self.homeolog_group_token_split_subgenome = bool(homeolog_group_token_split_subgenome)
        self.homeolog_group_token_cross_subgenome_attention = bool(homeolog_group_token_cross_subgenome_attention)
        if self.homeolog_group_token_split_subgenome and self.sg_channel_idx is None:
            logging.warning(
                "homeolog_group_token_split_subgenome requested but sg_channel_idx is missing; disabling split."
            )
            self.homeolog_group_token_split_subgenome = False
            self.homeolog_group_token_cross_subgenome_attention = False
        self.use_homeolog_auxiliary = bool(use_homeolog_auxiliary)
        self.homeolog_aux_stop_grad_target = bool(homeolog_aux_stop_grad_target)
        self.homeolog_aux_cosine_weight = float(homeolog_aux_cosine_weight)
        self.homeolog_aux_huber_weight = float(homeolog_aux_huber_weight)
        self.homeolog_aux_predictor: Optional[nn.Sequential] = None
        if self.use_homeolog_auxiliary and (
            self.hom_gid_channel_idx is None or self.sg_channel_idx is None
        ):
            logging.warning(
                "use_homeolog_auxiliary requested but hom_gid_channel_idx/sg_channel_idx is missing; disabling auxiliary loss."
            )
            self.use_homeolog_auxiliary = False
        if self.use_homeolog_auxiliary and (
            self.homeolog_aux_cosine_weight <= 0.0 and self.homeolog_aux_huber_weight <= 0.0
        ):
            logging.warning(
                "use_homeolog_auxiliary requested but all auxiliary component weights are non-positive; disabling auxiliary loss."
            )
            self.use_homeolog_auxiliary = False
        if self.use_homeolog_auxiliary:
            aux_hidden = max(8, int(homeolog_aux_hidden_dim or embed_dim))
            self.homeolog_aux_predictor = nn.Sequential(
                nn.Linear(embed_dim, aux_hidden),
                nn.LayerNorm(aux_hidden),
                nn.GELU(),
                nn.Linear(aux_hidden, embed_dim),
            )
        ...
        # Optional homeolog context injection (message passing over homology groups)
        self.homeolog_injector: Optional[HomeologContextInjector] = None
        if self.use_homeolog_context and (
            self.hom_has_channel_idx is not None
            and ((self.hom_gid_channel_idx is not None) or (self.hom_hash_indices is not None and len(self.hom_hash_indices) > 0))
        ):
            self.homeolog_injector = HomeologContextInjector(
                ...
                mode=self.homeolog_context_mode,
                dropout=float(homeolog_context_dropout),
                scale=self.homeolog_context_scale,
                use_loo=True,
                add_code_embed=self.homeolog_context_add_code_embed,
            )
        # Environment -to- SNP modulation (element-wise scaling before pooling).

        # Optional: second-level homeolog-group tokens (Graphormer-style)
        self.homeolog_group_token_layers: Optional[nn.ModuleList] = None
        if use_homeolog_group_tokens and (self.hom_gid_channel_idx is not None):
            hash_idx = self.hom_hash_indices if (homeolog_group_token_use_hash_id and self.hom_hash_indices) else None
            n_layers = max(1, int(homeolog_group_token_layers))
            self.homeolog_group_token_layers = nn.ModuleList([
                HomeologGroupTokenAttention(
                    ...
                    hom_gid_channel_idx=int(self.hom_gid_channel_idx),
                    sg_channel_idx=(int(self.sg_channel_idx) if self.sg_channel_idx is not None else None),
                    hom_hash_indices=hash_idx,
                    split_subgenome_tokens=self.homeolog_group_token_split_subgenome,
                    cross_subgenome_attention=self.homeolog_group_token_cross_subgenome_attention,
    ...
    def _compute_homeolog_auxiliary(
        self,
        x: torch.Tensor,
        feat: torch.Tensor,
        pad_mask: torch.Tensor
        ...
        aux: Dict[str, torch.Tensor] = {}
        if (
            not self.use_homeolog_auxiliary
            or self.homeolog_aux_predictor is None
            or self.hom_gid_channel_idx is None
            or self.sg_channel_idx is None
        ):
            return aux
        if x is None or feat is None or pad_mask is None:
            return aux
        ...
        hom_gid = feat[..., self.hom_gid_channel_idx].round().long()
        valid = (~pad_mask) & (hom_gid > 0)
        if self.hom_has_channel_idx is not None and self.hom_has_channel_idx < feat.size(-1):
            valid = valid & (feat[..., self.hom_has_channel_idx] > 0.5)
        if valid.sum().item() < 2:
            return aux

        sg = (feat[..., self.sg_channel_idx] > 0.5).long()
        flat_idx = valid.reshape(-1).nonzero(as_tuple=False).squeeze(1)
        if flat_idx.numel() == 0:
            return aux

        x_valid = x.reshape(-1, D)[flat_idx].float()
        gid_valid = hom_gid.reshape(-1)[flat_idx]
        sg_valid = sg.reshape(-1)[flat_idx]
        batch_idx = (flat_idx // L).long()
        vocab = int(gid_valid.max().item()) + 1
        combined_gid = gid_valid + batch_idx * vocab
        _, inv = torch.unique(combined_gid, return_inverse=True)
        ...
        mask0 = (sg_valid == 0)
        mask1 = ~mask0
        sum0 = torch.zeros((n_groups, D), device=x.device, dtype=torch.float32)
        cnt0 = torch.zeros((n_groups, 1), device=x.device, dtype=torch.float32)
        sum1 = torch.zeros((n_groups, D), device=x.device, dtype=torch.float32)
        cnt1 = torch.zeros((n_groups, 1), device=x.device, dtype=torch.float32)

        if mask0.any():
            sum0.index_add_(0, inv[mask0], x_valid[mask0])
            cnt0 += torch.bincount(inv[mask0], minlength=n_groups).to(torch.float32).unsqueeze(1).to(x.device)
        if mask1.any():
            sum1.index_add_(0, inv[mask1], x_valid[mask1])
            cnt1 += torch.bincount(inv[mask1], minlength=n_groups).to(torch.float32).unsqueeze(1).to(x.device)

        sum0_i = sum0[inv]
        cnt0_i = cnt0[inv]
        sum1_i = sum1[inv]
        cnt1_i = cnt1[inv]
        sg_f = sg_valid.view(-1, 1).to(torch.float32)
        other_sum = sum1_i * (1.0 - sg_f) + sum0_i * sg_f
        other_cnt = cnt1_i * (1.0 - sg_f) + cnt0_i * sg_f
        eligible = other_cnt.squeeze(1) > 0
        n_pairs = int(eligible.sum().item())
        if n_pairs <= 0:
            return aux

        pred = self.homeolog_aux_predictor(x_valid[eligible]).float()
        target = other_sum[eligible] / other_cnt[eligible].clamp_min(1.0)
        if self.homeolog_aux_stop_grad_target:
            target = target.detach()

        losses: List[torch.Tensor] = []
        if self.homeolog_aux_cosine_weight > 0.0:
            pred_norm = F.normalize(pred, dim=-1, eps=1e-6)
            target_norm = F.normalize(target, dim=-1, eps=1e-6)
            cosine_loss = 1.0 - (pred_norm * target_norm).sum(dim=-1)
            aux["homeolog_aux_cosine"] = cosine_loss.mean().detach()
            losses.append(self.homeolog_aux_cosine_weight * cosine_loss.mean())
        if self.homeolog_aux_huber_weight > 0.0:
            huber_loss = F.smooth_l1_loss(pred, target, reduction="none").mean(dim=-1)
            aux["homeolog_aux_huber"] = huber_loss.mean().detach()
            losses.append(self.homeolog_aux_huber_weight * huber_loss.mean())
        if not losses:
            return {}

        aux["homeolog_aux_loss"] = sum(losses)
        aux["homeolog_aux_pairs"] = pred.new_tensor(float(n_pairs)).detach()
        return aux
    ...
    def _encode_genomic(
        ...
        if self.use_habe:
            ...
            if bae_inputs is not None and self.genomic_bae is not None:
                ...
                proj_feat = feat
                if self.hom_gid_channel_idx is not None:
                    proj_feat = proj_feat.clone()
                    proj_feat[..., self.hom_gid_channel_idx] = 0.0
                x = self.genomic_feature_proj(proj_feat)
            if (
            ...
            if (
                ...
                meta_gamma = self.meta_to_gamma(meta_repr).unsqueeze(1) * self.meta_film_scale
                meta_beta = self.meta_to_beta(meta_repr).unsqueeze(1) * self.meta_film_scale
                x = x * (1.0 + meta_gamma) + meta_beta            # Inject cross-homeolog context at SNP level before block aggregation
            ...
            genomic_aux = self._compute_homeolog_auxiliary(x, feat, pad_mask)
            ...
            return g_repr, None, genomic_aux
    ...
    def forward(
        ...
        g_repr, per_chrom, genomic_aux = self._encode_genomic(genomic, mask, env_repr_main, env_seq_main, meta_repr)
    ...
    def encode_combined(self, genomic, mask, env_ts, loc_ids, year_ids, pop_ids, row_labels=None):
        ...
        pop_emb = self.pop_embedding(pop_ids)
        meta_repr = torch.cat([loc_emb, year_emb, pop_emb], dim=-1)
        g_repr, per_chrom, _ = self._encode_genomic(genomic, mask, env_repr_main, env_seq_main, meta_repr)
        fused = torch.cat([g_repr, env_repr_main, loc_emb, year_emb, pop_emb], dim=-1)
        self._last_per_chrom = per_chrom
