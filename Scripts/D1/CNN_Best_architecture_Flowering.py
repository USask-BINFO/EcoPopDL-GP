...
train_folder = 'Train'
test_folder = 'Test'
flo_data_csv = 'WCC_Flowering.csv'  # Path to the yield data CSV file
...
split_data_by_individual(source_folder, train_folder,test_folder, flo_data_csv)
...
class YieldDatasetWithMasks(Dataset):
    def __init__(self, csv_file, root_dir, mask_dir, transform=None, img_size=(224, 224), normalization="minmax"):
        ...
        self.data = self.data.dropna(subset=['Flowering'])
        self.data = self.data.reset_index(drop=True)
        self.data['Flowering'] = pd.to_numeric(self.data['Flowering'], errors='coerce').fillna(0)
        self.flo_min, self.flo_max = self.data['Flowering'].min(), self.data['Flowering'].max()
    ...
    def __getitem__(self, idx):
        ...
        # Yield values
        flo_ = (self.data.loc[idx, 'Flowering'] - self.flo_min) / (self.flo_max - self.flo_min)
        flo_value = torch.tensor([flo_], dtype=torch.float32)

        return image, mask, metadata, flo_value
...
class YieldNetWithMasks(nn.Module):
    def __init__(self, metadata_features=3):
...
def permutation_importance_r2(model, dataloader, feature_names, device="cpu", num_permutations=5):
    """
    Calculate permutation importance for metadata features based on R².

    Args:
    - model: Trained PyTorch model.
    - dataloader: DataLoader containing test data.
    - feature_names: List of metadata feature names.
    - device: Device for computation ("cpu" or "cuda").
    - num_permutations: Number of permutations to calculate importance.
    ...
    model.eval()
    baseline_r2_scores = []
    permuted_r2_scores = {feature: [] for feature in feature_names}

    # Collect predictions and R² for the baseline model
    with torch.no_grad():
        all_labels, all_preds = [], []
        for inputs, masks, metadata, labels in dataloader:
            inputs, masks, metadata, labels = (
                inputs.to(device),
                masks.to(device),
                metadata.to(device),
            ...
            preds = model(inputs, masks, metadata)
            all_labels.extend(labels.cpu().numpy())
            all_preds.extend(preds.cpu().detach().numpy())
        baseline_r2 = r2_score(all_labels, all_preds)

    # Permute each feature and measure the impact on R²
    for feature_idx, feature in enumerate(feature_names):
        for _ in range(num_permutations):
            permuted_labels, permuted_preds = [], []
            for inputs, masks, metadata, labels in dataloader:
                inputs, masks, metadata, labels = (
                    inputs.to(device),
                    masks.to(device),
                ...
                # Permute the feature
                original_feature = metadata[:, feature_idx].clone()
                metadata[:, feature_idx] = metadata[torch.randperm(metadata.size(0)), feature_idx]

                # Calculate predictions with permuted metadata
                preds = model(inputs, masks, metadata)
                permuted_labels.extend(labels.cpu().numpy())
                permuted_preds.extend(preds.cpu().detach().numpy())

                # Restore original metadata
                metadata[:, feature_idx] = original_feature

            # Calculate permuted R²
            permuted_r2 = r2_score(permuted_labels, permuted_preds)
            permuted_r2_scores[feature].append(permuted_r2)

    # Calculate the importance as the decrease in R²
    importance_scores = {
        feature: baseline_r2 - np.mean(permuted_r2_scores[feature])
    ...
    importance_df = pd.DataFrame({
        ...
        "Importance": [importance_scores[feature] for feature in feature_names]
...
# Assuming metadata feature names
metadata_feature_names = ["Location", "Year", "Pop"]

# Calculate permutation importance for R²
importance_df = permutation_importance_r2(
    model=YieldNetWithMasks().to(device),
    dataloader=test_loader,
    feature_names=metadata_feature_names,
    device=device
)

# Save and plot the feature importance
importance_df.to_csv("metadata_importance_r2.csv", index=False)
importance_df.plot(kind="barh", x="Feature", y="Importance", legend=False)
plt.title("Metadata Feature Importance (R²)")
plt.xlabel("R² Decrease")
plt.ylabel("Metadata Features")
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig("metadata_importance_r2_plot.png")
plt.show()
...
# Load the best model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = YieldNetWithMasks(metadata_features=3).to(device)
