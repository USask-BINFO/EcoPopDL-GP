...
train_folder = 'Train'
test_folder = 'Test'
yield_data_csv = 'WCC_SW.csv'  # Path to the yield data CSV file
...
class YieldDatasetWithMasks(Dataset):
    def __init__(self, csv_file, root_dir, mask_dir, transform=None, img_size=(224, 224), normalization="minmax"):
        ...
        self.data = self.data.dropna(subset=['SW'])
        self.data = self.data.reset_index(drop=True)
        self.data['SW'] = pd.to_numeric(self.data['SW'], errors='coerce').fillna(0)
        self.yield_min, self.yield_max = self.data['SW'].min(), self.data['SW'].max()
    ...
    def __getitem__(self, idx):
        ...
        # Yield values
        yield_ = (self.data.loc[idx, 'SW'] - self.yield_min) / (self.yield_max - self.yield_min)
        yield_value = torch.tensor([yield_], dtype=torch.float32)
...
extract_fc2_weights_with_metadata(
    ...
    dataset=test_dataset,  # Replace with your full dataset
    best_fold_model_path="best_model_fold3.pth",  # Path to the best fold's model
    save_path="best_fold_outputs_test.csv"
)
...
extract_fc1_outputs_with_metadata(
    ...
    dataset=test_dataset,  # Replace with your full dataset
    best_fold_model_path="best_model_fold3.pth",  # Path to the best fold's model
    save_path="fc1_outputs_test_fc1.csv"
)
