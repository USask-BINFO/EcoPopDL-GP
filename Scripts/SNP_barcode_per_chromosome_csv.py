...
def main():
    ...
    #read pruned_data_admixture.map
    pruned_data_admixture = 'pruned_data_admixture.map'
    # Process and transpose the SNP dataset with MAF filtering
    ...
    # Encode major and minor alleles
    major_minor_df = major_minor_allele(filtered_snps)
    major_minor_df_T = major_minor_df.transpose()
    #save major_minor_df_T to a csv file
    major_minor_df_T.to_csv('major_minor_df.csv')
    #append chr_id column to major_minor_df_T from file_name
    # Match and add 'Chr_id' to major_minor_df_T
    ...
    if major_minor_df_T is not None:
        for sample_name, sample_data in major_minor_df_T.iterrows():
            save_sample_as_image(sample_data, sample_name, output_folder)

# Extract the last row (chr_id_row) for chromosome data
    chr_id_row = major_minor_df_chr_T.iloc[-2]  # Assuming this is the chromosome data row
    locus_row = major_minor_df_chr_T.iloc[-1]  # Assuming this is the locus data row

    #skip last two rows of major_minor_df_chr_T
    major_minor_df_chr_T = major_minor_df_chr_T.iloc[:-2]

    output_folder_chr = os.path.expanduser('images_AF_chr')
    if not os.path.exists(output_folder_chr):
        os.makedirs(output_folder_chr)

    if major_minor_df_chr_T is not None:
        # Iterate over each sample row (excluding the chr_id_row)
        for sample_name, sample_data in major_minor_df_chr_T.iloc[:-1].iterrows():
            save_sample_as_image_chr(sample_data, sample_name, output_folder_chr, chr_id_row, locus_row)
