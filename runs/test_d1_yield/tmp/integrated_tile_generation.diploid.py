import logging
import numpy as np, math, os, pandas as pd, tempfile, zipfile, io
...
def safe_save_npz(path: str, **arrays):
    ...
    os.close(fd)
    try:
        # Write to memory first to ensure numpy actually emits bytes
        buf = io.BytesIO()
        np.savez_compressed(buf, **arrays)
        data = buf.getvalue()
        if len(data) == 0:
            raise RuntimeError("np.savez_compressed produced zero bytes")
        ...
        if tmp_size <= 0:
            logging.warning(f"Temp NPZ is zero bytes before replace: {tmp}")
        os.replace(tmp, path)
...
# Global variables
chr_info = {'Ca1': 1400000, 'Ca2': 2400000, 'Ca3': 3400000}
...
# Build a labeled chr_info so keys match 'A01','C01',...
# chr_info_labeled = {NUM_TO_LABEL.get(k, k): v for k, v in chr_info.items()}
...
def assert_subgenome_config_valid():
    if USE_SUBGENOMES:
        assert 'NUM_TO_LABEL' in globals() and NUM_TO_LABEL, \
            "In subgenome mode you must define NUM_TO_LABEL."
        assert SUBGENOME_CHRS and all(isinstance(v, list) and len(v) for v in SUBGENOME_CHRS.values()), \
            "Define SUBGENOME_CHRS = {'A':[...], 'C':[...]} (or A/B/D, etc.)."
...
TE_GENE_ANNOTATION_TSV = '/birl2/data/brassica/thulani/Research/CMPT898/CMPT-PLSC_819_Project/Github/EcoPopDL-GP/runs/test_d1_yield/04_tensors/te_annotation.from_gff.tsv'
SNP_TE_ANNOTATION_OUT = "snps_with_te_annotation.tsv"

# Gene annotation input/output (set to None to skip)
GENE_GFF_PATH = '/birl2/data/brassica/thulani/Research/CMPT898/CMPT-PLSC_819_Project/Github/EcoPopDL-GP/Scripts/Sample_data/cicar.CDCFrontier.gnm1.ann1.nRhs.gene_models_main.gff3'
...
def _normalize_numeric_chr_token(token: str) -> str | None:
    ...
    m = re.fullmatch(r"(?i)(?:chr|chromosome|ca)[_-]*0*([0-9]+)", t)
    if m:
        return str(int(m.group(1)))
    return None
...
def normalize_te_chr_label(label: str) -> str | None:
    """
    Normalize TE chromosome labels into canonical digits ('Ca8' -> '8').
...
def normalize_gene_chr_label(label: str) -> str | None:
    """
    Normalize gene seqids for chickpea by extracting CaN/chrN/numeric labels.
...
def build_chr_info_for_mode():
    ...
    if USE_SUBGENOMES:
        ...
        local = {NUM_TO_LABEL.get(k, k): v for k, v in chr_info.items()}
...
def load_gene_gff(
    ...
    df["end0"] = df["end"]
    return df[["chr", "chr_norm", "start", "end", "start0", "end0", "strand", "gene_id"]]
    ...
    if genes is not None and not genes.empty:
        try:
            ...
            gr_blocks = pr.PyRanges(
                blocks[["chr_norm", "start0", "end0", "block_id"]].rename(
                    columns={"chr_norm": "Chromosome", "start0": "Start", "end0": "End"}
                )
            )
            gr_genes = pr.PyRanges(
...
def compute_call_rate_per_snp(encoded_df: pd.DataFrame) -> pd.Series:
    """
    Vectorized per-SNP call-rate across samples (fraction of genotypes with 2 valid digits).
    Works on either ATCG-coded ('1'..'4') or major/minor-coded ('0'..'3') tables.
    Ignores annotation cols.
    """
    ...
    # --- NEW: dosage tables (numeric or 1-char strings) ---
    # If entries are numeric (or 1-char strings), treat non-NaN as "called".
    try:
        vals = encoded_df[sample_cols].apply(pd.to_numeric, errors='coerce')
        called = vals.notna().sum(axis=1)
        return called / max(1, len(sample_cols))
    except Exception:
        pass

    # ... existing byte/2-char path unchanged below ...
    ...
    # accept ASCII '0'..'4' (48..52) or '1'..'4' (49..52) → we use 48..52 to cover both
    valid = (b0 >= 48) & (b0 <= 52) & (b1 >= 48) & (b1 <= 52)
...
def _existing_W_from_npz(sample, sg_dir, label):
    ...
    if not files:
        return None
    try:
        meta = np.load(files[0], allow_pickle=False)["meta"]
...
def series_codes_to_dosage_norm(sample_series: np.ndarray) -> np.ndarray:
    ...
    is_digit_pair = (b0 >= 48) & (b0 <= 57) & (b1 >= 48) & (b1 <= 57)
...
def major_minor_allele(encoded_df: pd.DataFrame) -> pd.DataFrame:
    ...
    c0 = (b0 == 48).sum(axis=1) + (b1 == 48).sum(axis=1)
    c1 = (b0 == 49).sum(axis=1) + (b1 == 49).sum(axis=1)
    c2 = (b0 == 50).sum(axis=1) + (b1 == 50).sum(axis=1)
    c3 = (b0 == 51).sum(axis=1) + (b1 == 51).sum(axis=1)
    counts = np.stack([c0, c1, c2, c3], axis=1)               # shape (n_rows, 4)

    # Row-wise ranking of digits by descending frequency.
    # args: 0..3 correspond to digits '1','2','3','4'
...
# def compute_maf_per_snp_from_encoded(encoded_df):
#     """
#     Compute per-SNP MAF from a SNP x samples matrix where each cell is a two-character
#     nucleotide code like '12','34','-1','99'. Works before major/minor remapping.
#     encoded_df may contain 'Chromosome','Position','Haplotype_Block' columns; they are dropped.
#     Returns: pandas.Series indexed by SNP (row index of encoded_df).
#     """
#     ann = {'Chromosome', 'Position', 'Haplotype_Block', 'MAF', 'MA'}
#     sample_cols = [c for c in encoded_df.columns if c not in ann]
#     if not sample_cols:
...
#     # Valid if BOTH chars are 1..4 (ASCII 49..52); '-1'/'99' etc. are excluded
#     valid = (b0 >= 49) & (b0 <= 52) & (b1 >= 49) & (b1 <= 52)

#     c1 = (b0 == 49).sum(axis=1) + (b1 == 49).sum(axis=1)
#     c2 = (b0 == 50).sum(axis=1) + (b1 == 50).sum(axis=1)
#     c3 = (b0 == 51).sum(axis=1) + (b1 == 51).sum(axis=1)
#     c4 = (b0 == 52).sum(axis=1) + (b1 == 52).sum(axis=1)
#     counts = np.stack([c1, c2, c3, c4], axis=1).astype(np.float64)

#     denom = valid.sum(axis=1) * 2.0  # two alleles per valid genotype
#     with np.errstate(divide='ignore', invalid='ignore'):
#         freqs = counts / denom[:, None]

#     # For monomorphic rows, min positive freq would be 1.0; set those to 0.0 instead
#     num_pos = (counts > 0).sum(axis=1)
#     maf = np.where(
#         num_pos >= 2,
#         np.where(freqs > 0, freqs, np.inf).min(axis=1),
#         0.0
#     )
...
def filter_snps_by_maf_in_batches(encoded_df, maf_threshold=0, batch_size=1000):
    ...
    for start in range(0, encoded_df.shape[0], batch_size):
        ...
        maf_dict = calculate_maf_per_snp(batch)
        keep = [snp for snp, maf in maf_dict.items() if maf is not None and maf >= maf_threshold]
...
def instability_score(encoded_df: pd.DataFrame) -> pd.Series:
    ...
    def gap(row):
        # Accept 0..4 to support major/minor-coded tables; ignore other chars (e.g., '9')
        counts = {d: 0 for d in '01234'}
        for v in row.values:
            s = str(v)
            if s in ('-1', '99') or len(s) != 2:
                continue
        ...
        if tot == 0:
            return np.nan
        top = sorted((counts[d] for d in '01234'), reverse=True)
...
def add_haplotype_block_info(encoded_genotypes, haplotype_blocks):
    ...
    if pd.api.types.is_numeric_dtype(haplotype_blocks['CHR']):
        ...
        encoded_genotypes['Chromosome'] = encoded_genotypes['Chromosome'].astype(str)
        haplotype_blocks['CHR'] = haplotype_blocks['CHR'].astype(str)
        ...
        for rec in it:
            ...
            if use_DS_if_present and 'DS' in rec.format.keys():
                ...
                for gt in g:
                    ...
                    # any-alt dosage
                    dos.append(float((gt[0] or 0 != 0) + (gt[1] or 0 != 0)))
                rows.append(np.asarray(dos, dtype=np.float32))
        M = np.vstack(rows)
        df = pd.DataFrame(M, columns=sample_names, index=idx)
        df['Chromosome'] = chroms
...
def group_homology_by_label(homology_df: pd.DataFrame, id_normalizer):
    ...
    for r in homology_df.itertuples(index=False):
        ...
        gid = float(getattr(r, "hom_group_id"))
        pid = float(getattr(r, "hom_pair_id"))
...
def save_all_chromosomes_as_image(sample_data, sample_name, folder, chr_id_row, locus_row, dpi=100, max_channels=3):
    """Save combined chromosome image ensuring correct genomic spacing and handling overlaps with multiple channels."""
    
    strip_height = 10  # Increase visibility per chromosome
    total_chrs = len(chr_info)
    fig_height = strip_height * total_chrs

    # Define scale factor based on longest chromosome
    longest_chr_len = max(chr_info.values())
    desired_max_width_px = 5000  # Adjust based on visualization needs
    scale_factor = desired_max_width_px / longest_chr_len  # pixels per base pair

    fig_width_px = int(longest_chr_len * scale_factor)
    fig_width_in = fig_width_px / dpi

    # Initialize SNP image with multiple channels
    snp_image = np.zeros((total_chrs * strip_height, fig_width_px, max_channels), dtype=np.uint8)

    y_offset = 0
    for chr_index, chr_name in enumerate(sorted(chr_info.keys(), key=int)):
        chr_len = chr_info[chr_name]

        # Extract SNPs for chromosome
        chr_loci = np.array([locus for locus, chr_id in zip(locus_row, chr_id_row) if str(chr_id) == str(chr_name)])
        chr_alleles = [allele for locus, allele, chr_id in zip(locus_row, sample_data, chr_id_row) if str(chr_id) == str(chr_name)]

        chr_width_px = int(chr_len * scale_factor)

        # Convert genomic positions to pixel positions
        loci_scaled = (chr_loci * scale_factor).astype(int)

        pixel_counts = {}
        for locus_px, allele in zip(loci_scaled, chr_alleles):
            color = np.array(color_map.get(canonical_pair(allele), [200, 200, 200])) / 255

            
            if locus_px >= fig_width_px:  
                logging.warning(f"SNP at {locus_px} is out of bounds for {chr_name} (width {fig_width_px})")
                continue  # Ignore SNPs exceeding scaled width
            
            channel_idx = pixel_counts.get(locus_px, 0)
            if channel_idx < max_channels:
                snp_image[y_offset:y_offset + strip_height, locus_px, channel_idx] = color[0]
                pixel_counts[locus_px] = channel_idx + 1  # Increment for next SNP in same pixel
            else:
                logging.warning(f"Too many SNPs at pixel {locus_px} on {chr_name}, some SNPs ignored.")
    ...
    plt.imsave(image_path, snp_image.astype(np.uint8), cmap='viridis', dpi=dpi)
    
    logging.info(f"Saved corrected SNP image for {sample_name} at {image_path}")
...
def save_tiled_features_snp(
    ...
    if qc is None:
        ...
        valid = np.array([
            (isinstance(x, str) and len(x) == 2 and x[0] in '01234' and x[1] in '01234')
    ...
    for t in range(num_tiles):
        ...
        if include_density_channel and density_rasters is not None:
            ...
            tile_layers[t][0][:, :, dens_idx] = density_rasters[t]
...
def save_hierarchical_tensor(
    ...
    Z=feature channels) to NPZ. Channels: dosage, normalized locus, quality, optional
    TE/gene distances, TE flag, gene context, block summaries, homology/haplotype and subgenome flag.
    ...
    # Optional homology lookup
    hom_lookup = _build_homology_lookup_tables(homology_df, id_normalizer=id_normalizer)
    ...
    if has_hap_channels:
        ...
        ld_arr = np.asarray(ld_row, dtype=np.float32)
        ...
        # Persist raw block id per SNP; use -1 to mark "no block"
        hap_block_raw = np.where(haplo_arr > 0, haplo_arr, -1).astype(np.float32)
    ...
    include_homology_channels = bool(hom_lookup)
    include_te_channel = te_is_te is not None and len(te_is_te) == len(locus_row)
    ...
    if include_homology_channels:
        feat_names.extend(['hom_group_id', 'hom_pair_id'])
    if include_te_dist:
    ...
    attn_masks = {}
    block_attn_masks = {}  # same-block adjacency (optional)
    ...
    for row_idx, lab in enumerate(row_labels):
        ...
        if cached_attn is not None and cached_attn.shape == (n, n):
            ...
            attn_masks[str(lab)] = np.ones((n, n), dtype=np.uint8)
        ...
        if include_homology_channels:
            h_gid, h_pid = _lookup_homology_vectorized(hom_lookup, str(lab), positions_bp[row_idx, :n])
            row_tensor[:, ch] = h_gid; ch += 1
            row_tensor[:, ch] = h_pid; ch += 1
        if include_te_dist:
        ...
        if has_hap_channels:
            ...
            if cached_block_attn is not None and cached_block_attn.shape == (n, n):
                ...
                block_attn_masks[str(lab)] = (bids[:, None] == bids[None, :]).astype(np.uint8)
    ...
    write_and_verify_npz(
        ...
        ,
        **{f"attn_mask__{lab}": attn_masks[lab] for lab in attn_masks},
...
def save_tiled_features_haplo(
    ...
    for chrom in sorted_chr:
        ...
        for bp1, bp2, bid, kb in blocks_by_label.get(chrom, []):
            # map block ends into global pixel coords (0..desired-1); ensure at least 1 px width
            x1 = interp_bp_to_px(be, pe, int(bp1))
            x2 = max(x1 + 1, interp_bp_to_px(be, pe, int(bp2)))  # half-open [x1, x2)
    ...
    if use_homology:
        for chrom in sorted_chr:
            ...
            for bp1, bp2, gid, pid in hom_by_label.get(chrom, []):
                x1 = interp_bp_to_px(be, pe, int(bp1))
                x2 = max(x1+1, interp_bp_to_px(be, pe, int(bp2)))
...
def _clear_sample_tiles(sample_dir: str, sample_name: str):
    ...
    """
    try:
        for fp in glob.glob(os.path.join(sample_dir, f"{sample_name}_tile_*.npz")):
            try:
                os.remove(fp)
            except OSError:
                pass
...
def render_sample_lossless(
    ...
    if USE_SUBGENOMES:
        ...
        # Sanity checks
        sanity_checks_after_render(f"{sample}_A", sample_snp_dir_A, sample_hap_dir_A, expected_snps=len(locus_row), use_subgenomes=True)
        sanity_checks_after_render(f"{sample}_C", sample_snp_dir_C, sample_hap_dir_C, expected_snps=len(locus_row), use_subgenomes=True)
        return W
...
def main():
        # --- set paths first ---
    vcf_path = '/birl2/data/brassica/thulani/Research/CMPT898/CMPT-PLSC_819_Project/Github/EcoPopDL-GP/runs/test_d1_yield/00_genotype/final_genotype.vcf.gz'
    ped_file = '/birl2/data/brassica/thulani/Research/CMPT898/CMPT-PLSC_819_Project/Github/EcoPopDL-GP/runs/test_d1_yield/00_genotype/final_genotype.ped'
    map_file = '/birl2/data/brassica/thulani/Research/CMPT898/CMPT-PLSC_819_Project/Github/EcoPopDL-GP/runs/test_d1_yield/00_genotype/final_genotype.map'
    haplotype_block_file = '/birl2/data/brassica/thulani/Research/CMPT898/CMPT-PLSC_819_Project/Github/EcoPopDL-GP/runs/test_d1_yield/00_genotype/haplotype_blocks.blocks.det'
    ENCODED_WITH_BLOCKS = '/birl2/data/brassica/thulani/Research/CMPT898/CMPT-PLSC_819_Project/Github/EcoPopDL-GP/runs/test_d1_yield/04_tensors/encoded_genotypes_with_haplotype_blocks.csv'
    ...
    if COLOR_MODE == 'allele_combination':
        ...
        dosage_from_codes = encoded_to_dosage_df(encoded_df)
        maf_series = compute_maf_from_dosage(dosage_from_codes)
    else:
    ...
    .fillna(-1)           # <— key fix: allow “no block”
    .astype(int)
    .to_numpy()
)
    ...
    def normalize_te_for_mode(label: str) -> str | None:
        base = normalize_te_chr_label(label)
        if base is None:
            return None
        return id_norm(base)

    def normalize_gene_for_mode(label: str) -> str | None:
        base = normalize_gene_chr_label(label)
        if base is None:
            return None
        return id_norm(base)
    ...
    for lab in row_labels:
        ...
        if n == 0:
            continue
        attn_mask_cache[str(lab)] = np.ones((n, n), dtype=np.uint8)
        bids = haplo_row[idxs]
        block_attn_mask_cache[str(lab)] = (bids[:, None] == bids[None, :]).astype(np.uint8)
        left_change = np.ones(n, dtype=bool)
        right_change = np.ones(n, dtype=bool)
        if n > 1:
            left_change[1:] = bids[1:] != bids[:-1]
    ...
    out_folder_combined_tensor = '/birl2/data/brassica/thulani/Research/CMPT898/CMPT-PLSC_819_Project/Github/EcoPopDL-GP/runs/test_d1_yield/04_tensors/tensors'
        ...
        try:
            ...
            if not snp_done:
                save_hierarchical_tensor(
                    ...
                    block_attn_mask_cache=block_attn_mask_cache,
